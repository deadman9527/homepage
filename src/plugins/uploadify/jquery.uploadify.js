...
SWFUpload.prototype.initSettings = function() {
	...
	this.ensureDefault("preserve_relative_urls", true);
...
SWFUpload.prototype.getFlashHTML = function() {
    return['<div id="',this.settings.id,'" class="uploadify" style="height: ',this.settings.height,'px; width: ',this.settings.width,'px; border:solid 1px #ddd;background: #F7F7F7;text-align:center;" ><object style="position: absolute; margin-left:-65px; z-index: 1" id="', this.movieName, '" type="application/x-shockwave-flash" data="', this.settings.flash_url, '" width="', this.settings.button_width, '" height="', this.settings.button_height, '" class="swfupload">', '<param name="wmode" value="', this.settings.button_window_mode, '" />', '<param name="movie" value="', this.settings.flash_url, '" />', '<param name="quality" value="high" />', '<param name="menu" value="false" />', '<param name="allowScriptAccess" value="always" />', '<param name="flashvars" value="' + this.getFlashVars() + '" />', "</object></div>"].join("");
		...
		onSelect: function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('文件 "' + file.name + '" 已在上传队列中.\n您是否要替换当中队列?');
					if (!replaceQueueItem) {
				...
				switch (errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							...
							this.queueData.errorMsg += '\n已超出队列可容纳的限制 (' + settings.queueSizeLimit + ').';
						}
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 超出了大小限制范围 (' + settings.fileSizeLimit + ').';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 是空的.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 的类型不属于支持范围 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
